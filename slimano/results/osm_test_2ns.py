...
def run(name):
    ...
    nsi_id = osmc.schedule_nsi_instantiation('k8s_nst2', 'k8s_nsi2', 'openstack-atnog-4')

    osmc.instantiate_nsi('k8s_nst2', 'k8s_nsi2', nsi_id, 'openstack-atnog-4')
...
class OsmClient:
    ...
    def instantiate_nsi(self, nst_name, nsi_name, nsi_id, vim_name):
        ...
        if not res and not res.json().get('id'):
            print('an error has occurred during NSI instantiation. response: {}'.format(res.json()))
            return
...
if __name__ == '__main__':
    ...
    run_nr = 1
    while run_nr <= runs:
        run('test_osm_2ns_{}'.format(run_nr))
        run_nr = run_nr + 1

...
def deploy_nsi_1_ns(callback_ip):
    payload_nsi = {
        "slimano:nsi": {
            "name": "k8s-instance",
            "nst-name": "k8s-nst",
            ...
            "sdn-apps": [],
            "connections": [],
            "callback": "http://{}:8080/callback".format(callback_ip)
        }
    }
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        print('USAGE: python sm_test_10ns.py 192.168.85.237')
        exit(2)

    runs = 10
    run_nr = 1
    while run_nr <= runs:
        run('test_10ns_{}'.format(run_nr), sys.argv[1])
        run_nr = run_nr + 1
